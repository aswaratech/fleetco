// Vehicle-compliance expiry classification (ADR-0031 commitment 5 / §E),
// hosted in @fleetco/shared so BOTH apps consume ONE copy (ADR-0038 commitment
// 6 — the load-bearing drift guard). FleetCo stores compliance-document expiry
// dates (bluebook, insurance, route permit) as ISO/UTC on the Vehicle
// aggregate; this pure helper classifies one such date against "now" into the
// badge state the Vehicle detail page paints as an amber "Expiring soon" / red
// "Expired" <Badge> — AND the same state the notification scan (apps/api,
// Program C / ADR-0038) reads to decide what to email. It is the classification
// sibling of `nepali-date.ts`'s formatter — same module family (date display +
// date status), kept in its own file so the BS formatter stays purely about
// rendering.
//
// WHY THIS LIVES IN @fleetco/shared (ADR-0038 commitment 6): the reminder
// (apps/api scan) and the badge (apps/web page) MUST classify an item
// IDENTICALLY, or the operator sees a red "Expired" badge on the page but never
// gets the email (or vice-versa). The only way to guarantee that is one pure
// classifier both surfaces import — never a copy that can drift. This is the
// same anti-drift discipline the project enforces between the pino redact and
// the span-scrub lists, and between Tailwind and DESIGN.md: own the value that
// matters, don't fork it. apps/web re-exports these symbols from its own
// `lib/compliance.ts` so its existing `@/lib/compliance` importers are
// unchanged.
//
...
// apps/web/test/compliance.test.ts (which exercises these helpers through the
// re-export).
//
