...
// omits skip at page 0 so the canonical /fuel-logs URL stays clean.
...
// new column desc. Same convention as Jobs / Trips / Customers / Drivers.
function sortParams(
...
function SortableHeader({
  ...
  const href = `/fuel-logs${sortParams(searchParams, column, activeColumn, activeDir)}`;
...
function Pagination({ total, skip, take, searchParams }: PaginationProps): React.ReactElement {
  ...
  const prevHref = `/fuel-logs${paginationParams(searchParams, Math.max(0, skip - safeTake))}`;
  const nextHref = `/fuel-logs${paginationParams(searchParams, skip + safeTake)}`;
        ...
        {pages.map((p, idx) =>
          p === "ellipsis" ? (
            ...
            <Button key={p} asChild variant="ghost" size="sm" className="tabular-nums">
              <Link href={`/fuel-logs${paginationParams(searchParams, (p - 1) * safeTake)}`}>
                {p}
              </Link>
            </Button>
          ),
        ...
        <header className="flex items-end justify-between gap-4">
          <div className="space-y-1">
            <nav aria-label="Breadcrumb" className="text-text-muted text-sm">
              ...
              <span className="text-text-secondary">Fuel logs</span>
            </nav>
