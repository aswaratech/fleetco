// List-page skeleton for /jobs. Rendered by Next.js's Suspense boundary
// while page.tsx's server fetch is in flight — most commonly on a
// filter / sort / pagination change (the JobsFilters island calls
// router.push inside a transition). Matches the page layout (max-width,
// header block, one filter Select, table with N skeleton rows,
// pagination bar) so the transition is layout-stable. Mirrors
// apps/web/src/app/customers/loading.tsx with the Jobs column count
// (Job number, Customer, Status, Scheduled start = 4 columns).
