// List-page skeleton for /customers. Rendered by Next.js's Suspense
// boundary while page.tsx's server-side fetch is in flight — most
// commonly on a filter / sort / pagination change (the
// CustomersFilters client island calls router.push inside a
// transition, which triggers this loading state). Matches the page
// layout (max-width, header block, filter toolbar, table with N
// skeleton rows, pagination bar) so the transition is layout-stable.
//
// Mirrors apps/web/src/app/drivers/loading.tsx in structure with two
// adjustments for the Customers shape:
//   - one filter Select rather than two (status only — no
//     licenseClass / kind parallel).
//   - five table-row columns rather than six (Name, Contact, Phone,
//     Status, Created) so the skeleton bars line up with the rendered
    ...
    <main className="bg-surface-canvas min-h-svh" aria-busy="true">
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        ...
        {/* Filter toolbar skeleton — one select-shaped placeholder
            sized like the real trigger (w-44, h-9). The Customers
            surface has only the status filter; no licenseClass /
            kind parallel. */}
