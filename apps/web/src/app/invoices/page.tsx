...
// omits skip at page 0 so the canonical /invoices URL stays clean.
...
// Sortable-header link — toggles dir on the active column, else sets the new
// column desc. Same convention as Jobs / Customers / Trips.
function sortParams(
...
function SortableHeader({
  ...
  const href = `/invoices${sortParams(searchParams, column, activeColumn, activeDir)}`;
...
function Pagination({ total, skip, take, searchParams }: PaginationProps): React.ReactElement {
  ...
  const prevHref = `/invoices${paginationParams(searchParams, Math.max(0, skip - safeTake))}`;
  const nextHref = `/invoices${paginationParams(searchParams, skip + safeTake)}`;
        ...
        {pages.map((p, idx) =>
          p === "ellipsis" ? (
            ...
            <Button key={p} asChild variant="ghost" size="sm" className="tabular-nums">
              <Link href={`/invoices${paginationParams(searchParams, (p - 1) * safeTake)}`}>
                {p}
              </Link>
            </Button>
          ),
