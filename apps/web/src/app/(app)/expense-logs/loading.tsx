// List-page skeleton for /expense-logs. Rendered by Next.js's Suspense
// boundary while page.tsx's server fetch is in flight — most commonly
// on a filter / sort / pagination change (the ExpenseLogsFilters island
// calls router.push inside a transition). Matches the page layout
// (max-width, header block, three filters, table with N skeleton rows,
// pagination bar) so the transition is layout-stable. Mirrors
// apps/web/src/app/fuel-logs/loading.tsx with the Expense logs column
// count (Date, Vehicle, Trip, Category, Amount = 5 columns) and the
// extra category-select filter (3 filter inputs vs 2 in Fuel logs).
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        <header className="flex items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="bg-surface-muted h-8 w-44 animate-pulse rounded" aria-hidden="true" />
        ...
        <p className="sr-only">Loading expense logs…</p>
      </div>
    </main>
  );
}
