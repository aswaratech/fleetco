// List-page skeleton for /drivers. Rendered by Next.js's Suspense
// boundary while page.tsx's server-side fetch is in flight — most
// commonly on a filter / sort / pagination change (the DriversFilters
...
//
// Mirrors apps/web/src/app/vehicles/loading.tsx in structure; the only
// difference is the header skeleton is one element narrower because
// iter 6 does not render the "New driver" CTA (the iter-7 write path
// adds it).
    ...
    <main className="bg-surface-canvas min-h-svh" aria-busy="true">
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        ...
        {/* Filter toolbar skeleton — two select-shaped placeholders
            sized like the real triggers (w-44 / w-56, h-9). */}
        ...
        <p className="sr-only">Loading drivers…</p>
      </div>
    </main>
  );
}
