// List-page skeleton for /trackers. Rendered by Next.js's Suspense boundary
// while page.tsx's server-side fetch is in flight — most commonly on a
// filter / sort / pagination change. Matches the page layout (max-width,
// header block, filter toolbar, table with N skeleton rows, pagination bar)
// so the transition is layout-stable.
//
// Mirrors apps/web/src/app/(app)/geofences/loading.tsx with the Trackers
// shape: one filter Select (status) and five table columns (IMEI, Status,
// Vehicle, Label, Installed) so the skeleton bars line up with the rendered
    ...
    <main className="bg-surface-canvas min-h-svh" aria-busy="true">
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        ...
        {/* Filter toolbar skeleton — one select-shaped placeholder sized
            like the real trigger (w-44, h-9). The Trackers surface has
        ...
        <section className="border-border-subtle bg-surface-raised rounded border shadow-sm">
          ...
          {Array.from({ length: SKELETON_ROW_COUNT }).map((_, i) => (
            ...
            >
              ...
              <div className="bg-surface-muted h-5 w-16 animate-pulse rounded-full" />
        ...
        <p className="sr-only">Loading trackers…</p>
      </div>
    </main>
  );
}
