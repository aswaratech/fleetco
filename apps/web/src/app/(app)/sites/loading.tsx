// List-page skeleton for /sites. Rendered by Next.js's Suspense boundary while
// page.tsx's server-side fetch is in flight — most commonly on a filter / sort /
// pagination change (the SitesFilters client island calls router.push inside a
// transition, which triggers this loading state). Matches the page layout
// (max-width, header block, filter toolbar, table with N skeleton rows,
// pagination bar) so the transition is layout-stable.
//
// Mirrors apps/web/src/app/(app)/geofences/loading.tsx with the Sites shape:
// one filter Select (kind) and five table columns (Name, Kind, Location,
// Contact, Created) so the skeleton bars line up with the rendered table on
// first paint.
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        <header className="flex items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="bg-surface-muted h-8 w-24 animate-pulse rounded" aria-hidden="true" />
            <div className="bg-surface-muted h-4 w-24 animate-pulse rounded" aria-hidden="true" />
          </div>
        </header>

        {/* Filter toolbar skeleton — one select-shaped placeholder sized like
            the real trigger (w-44, h-9). The Sites surface has only the kind
            filter. */}
