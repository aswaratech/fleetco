...
// entirely so the canonical /trips URL stays clean.
...
// Build the link for a sortable column header. Same convention as the
...
function SortableHeader({
  ...
  const href = `/trips${sortParams(searchParams, column, activeColumn, activeDir)}`;
...
function Pagination({ total, skip, take, searchParams }: PaginationProps): React.ReactElement {
  ...
  const prevHref = `/trips${paginationParams(searchParams, Math.max(0, skip - safeTake))}`;
  const nextHref = `/trips${paginationParams(searchParams, skip + safeTake)}`;
        ...
        {pages.map((p, idx) =>
          p === "ellipsis" ? (
            ...
            <Button key={p} asChild variant="ghost" size="sm" className="tabular-nums">
              <Link href={`/trips${paginationParams(searchParams, (p - 1) * safeTake)}`}>{p}</Link>
            </Button>
          ),
        )}
        {nextDisabled ? (
        ...
        <header className="flex items-end justify-between gap-4">
          <div className="space-y-1">
            <nav aria-label="Breadcrumb" className="text-text-muted text-sm">
              ...
              <span className="text-text-secondary">Trips</span>
            </nav>
