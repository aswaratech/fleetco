// List-page skeleton for /geofences. Rendered by Next.js's Suspense
// boundary while page.tsx's server-side fetch is in flight — most commonly
// on a filter / sort / pagination change (the GeofencesFilters client
// island calls router.push inside a transition, which triggers this loading
// state). Matches the page layout (max-width, header block, filter toolbar,
// table with N skeleton rows, pagination bar) so the transition is
// layout-stable.
//
// Mirrors apps/web/src/app/customers/loading.tsx with the Geofences shape:
// one filter Select (type) and four table columns (Name, Type, Customer,
// Created) so the skeleton bars line up with the rendered table on first
// paint.
  ...
  return (
    <main className="bg-surface-canvas min-h-svh" aria-busy="true">
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        ...
        {/* Filter toolbar skeleton — one select-shaped placeholder sized
            like the real trigger (w-44, h-9). The Geofences surface has
            only the type filter. */}
        ...
        <p className="sr-only">Loading geofences…</p>
      </div>
    </main>
  );
}
