// List-page skeleton for /fuel-logs. Rendered by Next.js's Suspense
// boundary while page.tsx's server fetch is in flight — most commonly
// on a filter / sort / pagination change (the FuelLogsFilters island
// calls router.push inside a transition). Matches the page layout
// (max-width, header block, two date filters, table with N skeleton
// rows, pagination bar) so the transition is layout-stable. Mirrors
// apps/web/src/app/jobs/loading.tsx with the Fuel logs column count
// (Date, Vehicle, Liters, Price/L, Total = 5 columns).
  ...
  return (
    <main className="bg-surface-canvas min-h-svh" aria-busy="true">
      <div className="mx-auto max-w-6xl space-y-6 px-8 py-8">
        ...
        <p className="sr-only">Loading fuel logs…</p>
      </div>
    </main>
  );
}
