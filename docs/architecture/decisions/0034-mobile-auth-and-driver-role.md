...
**Annotation (2026-07-13, append-only — D6: the driver capability set of commitment 6 is complete):** `gps:read-derived` — the last capability of the lean driver set (c6) — is now GRANTED to the DRIVER role, honoring c5's hard rule by landing ATOMICALLY with its own-record scope: `TelematicsService.assertDriverCanReadVehicle` (a read-side twin of D4's `assertDriverCanIngest`) scopes a DRIVER to the derived status of the vehicle on their OWN `IN_PROGRESS` trip, and the fleet-wide `/positions/latest` route stays 403 for a DRIVER (`assertCanReadFleetPositions`). The DRIVER capability set is now exactly `{ trips:*, fuel-logs:*, gps:ingest, gps:read-derived }` — c6's lean set, reached incrementally (D2 → D4 → D6), each capability paired with its predicate in the same change. `gps:read-raw`, `geofences:read`, and every operational/admin token remain OFF the driver (the least-privilege posture holds). This is the ADR-0035 D6 slice; see ADR-0035's D6 annotation for the mechanism and the own-vehicle test matrix. No fresh ADR (ADR-0041 c2 in-phase completion).
