...
**Annotation (2026-07-05, append-only):** Two further changes. **ADR-0044** (third amendment) names a new egress class: uploaded document images — receipts, vendor bills — flowing as **pixels** to a hosted vision provider; the ADR-0043 text-redaction contract cannot apply to pixels, so the allowed document classes are enumerated there (identity documents governed by its Decision box A), attachment bytes are Tier-2-handled with 180-day transcript retention, and the provider choice is its Decision box B. **ADR-0045** resolves the financial-records revisit this ADR's §Revisit-when anticipated: no fifth tier — financial records stay Tier 3 with named handling rules (a provisional six-fiscal-year statutory retention floor, capture-aids-are-not-books-of-account, access-logging deferred with a named trigger).
