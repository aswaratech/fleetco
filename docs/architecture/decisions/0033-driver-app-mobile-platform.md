...
**Annotation (2026-07-10, append-only):** Commitment 7's deferred build-path decision is **discharged** — the D3→D4 boundary is reached and the PO chose **local prebuild** over EAS Build (decision 2026-07-10; ratified by the PR merging this annotation). Form: `npx expo prebuild --platform android` + Gradle on the operator's machine, with commitment 2's CNG discipline unchanged (the generated `android/` tree stays gitignored, disposable, never hand-edited); no EAS account or cloud-build dependency is adopted, and the operator/cost commitment priced in is the local toolchain (a Homebrew JDK 17 — the sudo-free `openjdk@17` formula, per the executed first build — plus the Android SDK command-line tools, ~6–10 GB) and its maintenance. The procedure — and commitment 4's owed **expo-doctor cadence** (manual: before any native dependency is added and before each prebuild; never in the polled CI gate) — lives in `docs/runbook/dev-setup.md` §"Building the driver app (local prebuild)". Release signing keystores are operator-generated Tier-1 secrets (ADR-0013): documented there, never created by an agent. D4–D6 resume under this build path per the same-day annotations on ADR-0035 and ADR-0042.
