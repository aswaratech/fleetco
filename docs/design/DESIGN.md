...
### Agent activity (addendum stub)

Reserved for ticket A8: a read-only **`/agent/activity`** page — the standing answer to "what did the agent do last week" (ADR-0043 c5). It reads `AgentAction` rows only (never transcripts), as a standard [Tables](#tables) list — date, tool, status `<Badge>`, latency, acting user, entity deep-link — filterable by date/tool/status. Full specification lands here with A8; this stub reserves the surface name and route so the A6 chat surface and the A8 audit surface are understood as the two halves of the agent's UI: the conversation, and the ledger.
