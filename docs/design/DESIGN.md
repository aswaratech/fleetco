...
- **Subtitle & BS calendar.** The window subtitle renders the resolved `from` / `to` through `<NepaliDate>` / `formatNepaliDate` (the `bs` variant, keeping the sentence compact), e.g. "5 vehicles with activity from `<BS>` to `<BS>`." The date **inputs** stay native AD per ADR-0031 commitment 6 — the BS date-picker input widget is a separate deferred slice; only the *display* subtitle is BS here, exactly as on the cost report.
