...
| Breadcrumb | Built | `ui/breadcrumb.tsx` |
...
| Command palette (cmdk) | Built | `ui/command.tsx` (mounted in `components/app-shell/`) |
...
- **2026-06-24 (Phase 1 T4 — breadcrumb + reports index):** `ui/breadcrumb.tsx` added — a hand-written primitive (no Radix, no new dependency) reproducing the inline breadcrumb pattern the pages carried byte-for-byte (`nav[aria-label="Breadcrumb"]`, `text-text-muted text-sm`, "›" separators in an aria-hidden span, linked crumbs `hover:text-text-primary`, the current crumb `text-text-secondary`), so migrating the existing inline breadcrumbs onto it (in the T6 sweep) is a null visual diff. It consumes only `@theme` utilities (the consumption guard scans it). Also adds the `/reports` index surface (a navigable-card list of the two reports — see §Surfaces "Reports index") and archives the now-shipped `app-shell.html` mockup to `slices/_archive/` (ADR-0008). Discharges the Breadcrumb contract-only row above.
- **2026-06-25 (Phase 1 T7 — ⌘K command palette):** `ui/command.tsx` added — the shadcn-ui cmdk-backed `Command` set (`Command` / `CommandDialog` / `CommandInput` / `CommandList` / `CommandEmpty` / `CommandGroup` / `CommandItem` / `CommandSeparator` / `CommandShortcut`), fetched from `https://raw.githubusercontent.com/shadcn-ui/ui/main/apps/v4/registry/new-york-v4/ui/command.tsx` (2026-06-25). **`cmdk@^1.1.1` is the one new top-level dependency** this ticket adds (ADR-0040); React-19 support was verified at add time (peer range `react: ^18 || ^19 || ^19.0.0-rc`; the lockfile resolved it against `react@19.2.6` — the react-leaflet-v5 check). Re-pointed from shadcn's dead `:root` aliases to FleetCo's `@theme` utilities (the consumption guard scans it): `bg-popover` → `bg-surface-elevated`, `text-popover-foreground`/`text-foreground` → `text-text-primary`, `text-muted-foreground` → `text-text-muted`, `data-[selected=true]:bg-accent` → `bg-surface-muted`, `data-[selected=true]:text-accent-foreground` → `text-text-primary` (the white-text trap), `bg-border` → `bg-border-subtle`; surface `rounded-lg`, controls `rounded`. The modal wrapper is built on the umbrella `radix-ui@1.4.3` Dialog (no separate dialog dependency; the standalone `Dialog` row stays contract-only), and the mockup's navigation-hints footer is composed in the shell. Mounted in `components/app-shell/app-shell.tsx` (the ⌘/Ctrl+K listener + the top-bar "Search…" affordance), sourcing `navForRole(role)`. No new design token — the design-token-drift test is untouched. Discharges the new "Command palette (cmdk)" Built row and closes the Phase-1 app-shell program.
...
- **shadcn-ui upstream:** `Command` (cmdk-backed); see [Component status](#component-status-built-vs-contract). Built in `ui/command.tsx`, mounted by the navigation shell (`components/app-shell/`). The locked mockup's **Frame 2** (`docs/design/slices/_archive/app-shell.html`) is the visual contract.
- **Invocation:** a global `⌘K` / `Ctrl+K` listener opens it from anywhere, plus a quiet top-bar "Search… ⌘K" affordance (a muted field: `color.surface.canvas` fill, `color.border.strong` border, `radius.default`, a `⌘`/`K` kbd chip).
- **Surface:** a centered modal (`560px`, `radius.lg` / 8px, `color.surface.elevated`, `color.border.subtle` border, raised elevation) over a 40%-black backdrop — a 48px search row (`color.text.muted` icon + placeholder "Go to…"), a scrolling list (max 320px), and a footer with the `↑↓ navigate · ↵ open · esc close` hints.
- **Content:** the **same `lib/nav.ts` source** the sidebar uses — `navForRole(role)`'s five groups (Operations / Money / Maintenance / Reports / Logs), each item a Lucide icon + label. The highlighted (`data-selected`) row is `color.surface.muted` + `color.text.primary` (never the white `color.accent.foreground`). Selecting a row navigates (`router.push`) and closes.
- **Scope:** navigation-only in v1 — it lists pages, not records. Resource/data search (jump to a specific vehicle or trip) is a later-phase extension on the same primitive (ADR-0040).
- **Modal note:** the dialog wrapper is built on the installed `radix-ui` Dialog inside `command.tsx`; the standalone `Dialog` primitive stays contract-only (ADR-0040).
...
- **Money:** `<Money paisa={integer} />` component (Built — `components/money.tsx` over `formatNpr` in `lib/money.ts`; see [Component status](#component-status-built-vs-contract)). Output: `Rs. ` + Intl-grouped integer rupees (en-IN locale, Nepali lakh grouping) + `.` + 2-digit paisa. Examples: `12550025` paisa → `"Rs. 1,25,500.25"`; `100` paisa → `"Rs. 1.00"`; `0` paisa → `"Rs. 0.00"`. Right-aligned in tables; `tabular-nums` applied. Negative amounts use parentheses: `-125000` paisa → `"(Rs. 1,250.00)"`. See [NPR / paisa display](#npr--paisa-display) for the full specification.
...
- **Purpose.** The landing surface at `/reports` for the Reports nav group — a short index of the available reports and a clean ⌘K palette target. Before Phase-1 T4 there was no `/reports` page, only the two leaf reports beneath it.
- **Shape.** A server component behind the auth gate. The standard container (`<main class="bg-surface-canvas min-h-svh">`, `mx-auto max-w-6xl px-8 py-8`), a `<Breadcrumb>` (`FleetCo › Reports`) above an `text-2xl font-semibold` title, then a responsive grid (`sm:grid-cols-2`) of **navigable cards** — each the section idiom (`border-border-subtle bg-surface-raised rounded border p-4 shadow-sm`) wrapped as a single `<Link>` so the whole card is the click target (no inner button — anti-pattern #3), with the report title (`text-lg`, `font-semibold`) and a one-line `color.text.muted` description. The two entries are **Cost report** (`/reports/per-vehicle-cost`) and **Fuel efficiency** (`/reports/per-vehicle-efficiency`).
- **No new tokens or components.** Reuses the shipped `<Breadcrumb>` and the section idiom; introduces no design token.
...
- **Purpose.** The at-a-glance "where is the fleet right now" surface at `/map` (ADR-0042, ticket M9): every tracked vehicle's **latest fix** as a marker on a map of the operating area, the **yard** drawn from the DEPOT geofence(s), and an honest **fix-age** treatment so a silent tracker reads as "stale/unknown," never as a false "parked here." It is the read-side consumer that makes the hardware-tracker investment observable — a monitoring instrument, not a route-history browser: it renders **one fix per vehicle, never trails** (raw trails are Tier-5, ADMIN-only, and belong to a future dedicated surface — ADR-0027 c6/c7).
- **Route, gate & nav.** `/map` inside the `(app)` shell, behind the auth gate. The data endpoint (`GET /api/v1/telematics/positions/latest`, M7) is `gps:read-derived`, so the surface serves **ADMIN + OFFICE_STAFF**; the "Live map" `NavItem` joins the **Operations** group in `lib/nav.ts` (the sidebar, quick-links, and ⌘K palette inherit it from the one source). The page header is the standard breadcrumb-above-title (`FleetCo › Live map`), then the map fills the remaining viewport height (`min-h-[70vh]`-class full-bleed within the standard container — the map is the content; do not box it into a card).
- **Map stack.** The already-sanctioned Leaflet stack exactly as the geofence editor (`apps/web/src/app/(app)/geofences/geofence-map-editor.tsx`) ships it (ADR-0030 c8: `leaflet` + `react-leaflet` + OSM raster tiles with the required attribution control, Kathmandu default view), loaded as a client-only island via `next/dynamic` `{ ssr: false }` (Leaflet touches `window` at module load — the established App Router gotcha). **No new dependency**; the read-only map draws no Geoman toolbar. Initial bounds: fit to the union of markers + yard polygons; fall back to the Kathmandu default when nothing renders.
- **Markers & fix age (the load-bearing honesty rule).** Each vehicle with a fix renders one marker whose state is the **server-computed** `fixAgeSeconds` (never the client clock):
  - **fresh** — age < 2 min → the accent hue (`color.accent.primary`): reporting now.
  - **aging** — 2 min ≤ age < 15 min → no special hue (the neutral marker): normal between-report gap.
  - **stale** — 15 min ≤ age < 24 h → `color.status.warning`: the tracker has gone quiet; the position is where it *was*.
  - **dead** — age ≥ 24 h → `color.status.neutral` (muted): do not trust this position.

  Every marker's popup and the sidebar row state the age in words ("2 min ago", "3 h ago") — the hue is recognition, the label is meaning (anti-pattern #2's rule applied to markers). The thresholds are ratified by this section merging (ADR-0042's open pick) and live as named constants in one pure helper (`markerStateForAge`) beside the fix-age formatting, unit-tested like `complianceBadgeState` — the map island itself stays thin.
- **Popup.** Registration (`font-mono`, deep-link `/vehicles/<id>`), vehicle kind, speed (km/h, converted at render from the stored m/s, one decimal, em-dash when null), ignition ("On" / "Off" / em-dash), and the fix age + `<NepaliDate>`-style timestamp line. No coordinates are printed as text — they are already ON the map; a printed lat/lon line invites copy-paste into chat/logs for zero operator value.
- **Sidebar / legend.** A compact panel beside (stacked below on small screens) the map: the marker-state legend, a "last updated HH:MM:SS" line for the poll itself, and the **untracked list** — vehicles with no ACTIVE tracker or no fix yet, as `registration · "No tracker"` / `· "No fix yet"` rows (they must not silently vanish from the fleet picture; absence-of-data is data). Non-retired vehicles only.
- **Refresh.** Client polling of the latest-positions endpoint every **20 s**, paused while `document.visibilityState` is hidden (an always-open tab must not hammer the API or the OSM tiles), resumed-and-fired on visibility return. No SSE/WebSocket — a realtime channel is a new cross-cutting pattern gated on its own ADR if 20 s proves too slow (ADR-0042 "Revisit when"). A failed poll keeps the last markers and shows the [Voice](#voice-and-tone) network line ("Cannot reach the server. Retry.") in the sidebar without blanking the map.
- **The yard (DEPOT overlay).** All `GeofenceType.DEPOT` fences, fetched once per page load from the existing geofences API and drawn as read-only polygons via the shipped WKT↔LatLng serializers (`lib/geofence-latlng.ts`): `color.accent.primary` outline, ~10% fill opacity, non-interactive except a name tooltip. CUSTOMER_SITE / ROUTE_CORRIDOR fences are **not** drawn in v1 (visual noise before value; a layer toggle is a future refinement).
- **Empty & edge states.** Per [Voice](#voice-and-tone), stated as fact: no trackers registered → the map renders yard-only with "No trackers installed." in the sidebar; trackers exist but no fixes yet → "No positions received yet."; empty fleet → "No vehicles registered." The surface ships a `loading.tsx` skeleton (a map-shaped `animate-pulse` block + sidebar rows) and the standard error boundary.
- **Privacy & egress discipline (Tier 5).** Coordinates render on the authorized caller's map and go nowhere else: never into the URL (no `?lat=` deep-links), never into client logging, never more than the single latest fix per vehicle. The endpoint enforces the same server-side (no geometry column, no trails, `fixAgeSeconds` computed server-side per ADR-0042 c10).
- **No new tokens or components.** Marker states bind to the existing `color.accent.primary` / `color.status.*` tokens; the panel reuses the section idiom + `<Badge>`; the only new code shapes are the thin map island and the pure `markerStateForAge` helper. `globals.css`'s `@theme` is untouched; the [design-token-drift test](#how-this-file-relates-to-code) and the app-wide consumption sweep stay green.
- **Anti-patterns referenced:** #2 (marker hue is recognition; the age label is the meaning), #3 (the sidebar rows carry one contextual deep-link each), #4 (no gradients — flat marker fills), #7 (state hue lives in the marker/badge, never a row wash), #15 (no coordinates or PII in URLs).

### Agent chat

- **Purpose.** The conversational surface at `/chat` (ADR-0043, ticket A6) where the operator talks to the AI agent: fleet questions ("how many trips this week?"), reports ("this month's fuel cost"), and — once the A7/A8 write tools land — dictated data entry ("register this driver…"). The agent operates the software through its curated tool registry as the requesting user; this surface is the conversation, the record of what ran, and nothing else. It is an instrument with a transcript, not a chatbot with a personality.
- **Route, gate & nav.** `/chat` inside the `(app)` shell, behind the auth gate. The endpoints (`/api/v1/agent/*`, ticket A5) are `agent:use`-gated — **ADMIN-only in v1** (ADR-0043 c1) — so this is the app's **first ADMIN-only surface**: the "Agent" `NavItem` joins the **Operations** group in `lib/nav.ts` with `allowedRoles: ["ADMIN"]` (the one-line change the nav model's role field was built for; the sidebar, quick-links strip, and ⌘K palette all inherit the gate from that single source). Lucide `Bot` icon. A non-ADMIN who navigates directly gets the API's 403, rendered as the plain fact "The agent is available to the ADMIN role only." — no redirect, no apology (per [Voice](#voice-and-tone)). Header: standard breadcrumb-above-title (`FleetCo › Agent`).
- **Layout — rail + transcript.** Two panes inside the standard container, mirroring the Live map's panel arrangement (side-by-side, stacked on small screens): a **conversation rail** (fixed-width left column) and the **transcript column** (the remaining width) with the composer pinned beneath the transcript. No third pane, no settings surface — model/provider configuration is env-level (ADR-0043 c2), not operator UI.
- **Conversation rail.** A "New conversation" `<Button>` above a list of the user's OWN conversations (the API scopes to the owner), most recently active first: each row is the conversation title (derived server-side from the first user message; an untitled just-created row renders "New conversation" muted) in `text-sm`, with a `text-xs` `color.text.muted` `<NepaliDate format="bs">`-style last-activity date. The active conversation carries the sidebar's selected treatment (`aria-current`, `bg-surface-sunken`); selection rides the URL as `?c=<conversation-id>` — an id is Tier-4, but message CONTENT never enters a URL (anti-pattern #15 applied to transcripts). Empty rail: "No conversations." above the button.
- **Transcript.** Insertion-ordered message blocks, `max-w` bounded for readability:
  - **User messages** — right-aligned blocks in the section idiom (`bg-surface-raised rounded border border-border-subtle p-3`), preserving line breaks.
  - **Assistant messages** — left-aligned plain prose (no bubble; the assistant is the instrument's output, not a chat partner). Rounds that carried only tool calls persist with empty content and render nothing themselves — the action cards below carry that information. App paths the assistant names (e.g. `/vehicles/<id>`, per its system-prompt rule) are linkified by a pure allowlist helper that converts **only known app routes** to `<Link>`s — model text is untrusted input, so arbitrary URLs/schemes never become clickable (the prompt-injection posture applied to rendering).
  - **System notices** (budget exhaustion, provider failure — server-authored) — a centered `text-xs` `color.text.muted` line. Facts, not apologies: "Turn stopped: the language-model call failed (http_500). Try again."
  - **Action cards (the c4c honesty rule).** Every tool dispatch renders a compact card sourced from the server's `AgentAction` row — **never from model text**: the tool name (`font-mono text-xs`), a status `<Badge>` (`success` succeeded / `error` failed / `neutral` denied), and the latency in ms (`tabular-nums`). A "Details" `<details>` disclosure reveals the recorded args as pretty-printed JSON — it is the operator's own data, and seeing exactly what ran is the point. Stage one (read tools) carries no entity deep-link; when A7/A8 populate `resultEntityType/Id`, the card gains its one contextual link to the affected record (anti-pattern #3: one link, no button cluster).
- **Composer.** A `<Textarea>` (the 8,000-char API bound mirrored client-side) + a "Send" `<Button>`; Enter sends, Shift+Enter inserts a newline. While a turn is pending the composer and Send disable (the client half of the per-conversation in-flight lock; the server 409s a concurrent turn regardless) and a muted status line states the fact: "Waiting for the agent. This can take up to a minute." — v1 is **non-streamed request/response** through a server action (ADR-0043 c7: the repo has no SSE infrastructure; the ~150 s web action budget sits above the API's 90 s turn wall-clock; token streaming is the named tech-debt follow-up). A 409 renders "A turn is already running for this conversation."; a failed action renders the [Voice](#voice-and-tone) network line. Empty transcript state: "Ask about the fleet — vehicles, trips, fuel, costs, maintenance."
- **Attachments (ADR-0044).** The composer gains a paperclip `<Button variant="ghost">` (Lucide `Paperclip`) beside Send, backed by a hidden `<input type="file" accept="image/jpeg,image/png,image/webp">`. On pick the client downscales (canvas re-encode, longest edge ~2048 px JPEG) and uploads through a server action; a **pending chip** above the composer shows a small thumbnail, the size ("1.2 MB"), and a remove ×; while uploading it reads "Uploading photo…" and Send disables. Send posts `{content, attachmentId}`; the sent user message renders its thumbnail (bounded height, `rounded border border-border-subtle`) above the text, and clicking opens the full image through the authed proxy route — attachment bytes are Tier 2 and never get a public URL. One attachment per turn in v1. The server's extraction record is a normal **system notice** line; the agent's field-by-field proposal is a normal assistant message — no new message shapes. Copy per [Voice](#voice-and-tone): "Photo is larger than 10 MB." · "Attach fuel or expense receipts, or vendor bills." · the identity-document refusal: "Identity documents (licenses, Bluebook owner pages) can't be processed. Enter those fields by typing." No new tokens; the chip and thumbnail are arrangements of existing primitives.
- **Privacy & egress discipline (Tier 2).** Transcript content is Tier 2 (ADR-0043 c6): it renders for the authorized owner and goes nowhere else — never into client logging or `console.*`, never into URLs (only the conversation **id** rides `?c=`), and the honest limit stated in the ADR applies here: everything typed in the composer reaches the hosted provider verbatim. The surface adds no client-side analytics or third-party embeds.
- **Loading & error states.** A `loading.tsx` skeleton (rail rows + transcript-shaped `animate-pulse` blocks) and the standard error boundary. A turn that returns messages plus a system notice is a NORMAL render (the transcript explains itself), not an error state.
- **No new tokens or components.** The surface is an arrangement of `<Button>`, `<Textarea>`, `<Badge>`, `<Breadcrumb>`, `<NepaliDate>`, the section idiom, and the skeleton pattern; the only new code shapes are the thin chat island, the server actions, and pure helpers (route-allowlist linkification, latency/label formatting) unit-tested like `markerStateForAge`. `globals.css`'s `@theme` is untouched.
- **Anti-patterns referenced:** #2 (status hue rides the `<Badge>`, the status word is the meaning), #3 (action cards carry at most one contextual link), #4 (no gradients — flat blocks), #7 (no row washes), #15 (no message content or PII in URLs).

### Agent activity

- **Purpose.** The read-only audit ledger at `/agent/activity` (ADR-0043 c5, ticket A8): the standing answer to "what did the agent do last week". It reads `AgentAction` rows only — never transcripts — and it is **cross-user** where `/chat` is owner-scoped: a conversation is one person's working context; the ledger is the organization's audit trail. Action rows are kept indefinitely and outlive their pruned transcripts (the `SetNull` detach), so this surface stays complete after the 180-day transcript prune. Chat and activity are the two halves of the agent's UI: the conversation, and the ledger.
- **Route, gate & nav.** `/agent/activity` in the `(app)` shell, behind the auth gate. The data endpoint (`GET /api/v1/agent/actions`) rides the agent controller's existing class-level `agent:use` gate — **ADMIN-only in v1**; no second token is minted while the audience is identical. The recorded caveat: if `agent:use` is ever granted to OFFICE_STAFF (the ADR's "Revisit when"), this CROSS-USER view widens with it — mint a dedicated audit token at that moment, not before. The "Agent activity" `NavItem` joins the **Logs** group beside "Reminder history" (the ledger framing — this is the agent's `NotificationLog` analog), Lucide `History` icon, `allowedRoles: ["ADMIN"]`. A non-ADMIN navigating directly gets the API's 403 rendered as the plain fact: "Agent activity is available to the ADMIN role only." Breadcrumb `FleetCo › Agent activity`.
- **Layout & columns.** The Reminder-history shape: header → filter toolbar → a bordered [Tables](#tables) section → pagination (take 20). Columns: **When** (`<NepaliDate format="bs">`, the one `SortableHeader`, createdAt desc default with id tiebreaker) · **Tool** (`font-mono text-xs`, e.g. `create_vehicle`) · **Status** (`<Badge>` via the shipped `actionBadgeVariant` — success/error/neutral, fail-closed on unknown values) · **Latency** (`formatLatencyMs`, `tabular-nums`) · **User** (the acting human's name, email as the title/fallback — ADR-0043 c1: agent-ness is the row, attribution is the human) · **Entity** (ONE deep-link via the shipped `entityPathFor(resultEntityType, resultEntityId)`; em-dash when null — reads/lists/reports affect no single entity) · **Details** (a `<details>` disclosure revealing the recorded `argsJson` and, when present, `previousJson` as pretty-printed JSON — the chat action card's idiom; it is the operator's own data, and reading the pre-image here IS the undo procedure's first step).
- **Filters.** Three, all riding URL searchParams (bookmarkable; a filter change resets `skip`): a status `Select` (All / succeeded / failed / denied — the `__all__` sentinel; the API filter stays an open string per the NotificationLog forward-compat rule, so an unknown value returns zero rows, never a 400), a **tool** exact-match `<Input>` (committed on Enter/blur; there is no web-side registry to enumerate, and the ADMIN knows the `snake_case` names from the cards), and a From/To `<NepaliDatePicker>` pair (the per-vehicle-cost precedent) mapping to `startDate`/`endDate` with the end date inclusive through end-of-day.
- **Empty & edge states.** Per [Voice](#voice-and-tone), stated as fact: "No agent actions yet." unfiltered; "No actions match the current filter." filtered. A `denied` row is a NORMAL render — the ledger has no silent paths (a capability refusal is exactly what an auditor wants to see). Ships a `loading.tsx` skeleton (header + toolbar + seven-column pulse rows) and uses the `(app)` error boundary.
- **Privacy & Tier-2 discipline.** `argsJson` and `previousJson` are Tier 2: they render for the authorized ADMIN and go nowhere else — never into client logging, never into URLs (only ids and filter tokens ride searchParams; anti-pattern #15). `previousJson` is the RAW pre-image, deliberately unredacted: redaction is a MODEL-context boundary, not an operator boundary — the ADMIN can read the underlying records anyway, and a redacted pre-image could not be restored from. Message content never appears on this surface at all (actions, not transcripts).
...
- **Library choice:** **decided and shipped.** ADR-0031 picked `nepali-date-converter` (exact-pinned — the app leans on its calendar table), with the FleetCo-owned `BS_MONTHS` month-name array and the UTC-calendar-day rule living in `packages/shared` (`formatNepaliDate`). The `<NepaliDate>` display component (ADR-0031) and the hand-built `<NepaliDatePicker>` input widget (ADR-0032) implement the contract and calendar-widget pattern documented above; every date field in the app renders and collects through them.
