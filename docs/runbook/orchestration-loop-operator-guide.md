...
| `next_prompt_missing` | All four extractor tiers (incl. the PR-body fallback) returned NONE; loop halted. | Inspect `logs/<latest>.log` to see the agent's actual output. Either the agent forgot to include a next-session prompt (write the next kickoff manually and restart) or the output format drifted in a new way (note it as a tech-debt entry to improve the extractor). |
| `next_prompt_too_short` | A next-prompt was extracted but is below the 4000-char floor; loop halted. Almost always an agent that compressed the kickoff, dropping the structural / hardening sections. | Inspect the extracted prompt in `logs/<latest>.log`. Write a complete kickoff manually (carry the full hardening sections verbatim — see the design doc's "Known agent failure modes & kickoff hardening") and relaunch. Do NOT relaunch with the thinned prompt. |
...
**No next prompt extracted.** All four extractor tiers (including the PR-body fallback) returned NONE. Either the agent forgot to draft a next prompt (rare; usually means the program is informally done — start a new program), or the agent's output format drifted in a new way none of the tiers handles (file as tech-debt; improve the extractor).

**Next prompt too short.** A prompt was extracted but fell below the 4000-char floor — distinct from "no next prompt." This almost always means the agent compressed the kickoff and dropped the structural / hardening sections (the "About Claude Code system reminders", "Do not enter plan mode", and commit-discipline sections). Relaunching with the thinned prompt would re-open the failure those sections prevent, so the loop refuses. Write a complete kickoff by hand — start from `kickoff.md.template` (which carries the hardening sections) and fill in the ticket — then relaunch. See the design doc's **"Known agent failure modes & kickoff hardening"** for why these sections are load-bearing and must never be compressed.

**Agent refused / entered plan mode (no PR detected).** If `logs/<latest>.log` shows the agent drafting a refusal to edit first-party code, or entering plan mode and waiting, that is a known agent failure mode (not a loop bug) — see the design doc's "Known agent failure modes & kickoff hardening." Recovery is the salvage pattern: the agent's committed checkpoints are already on the feature branch (commit-early discipline), so finish the in-flight checkpoint by hand, open the PR, and relaunch with a kickoff that carries the full hardening sections verbatim.
