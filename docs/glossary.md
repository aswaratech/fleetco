...
**Soft delete** is the pattern of marking a row as deleted (typically via a `deletedAt` timestamp) and filtering it out of queries, rather than physically removing it. Soft delete preserves referential integrity for related rows and supports recovery from accidental deletion. As of Phase 1 iter 8, FleetCo does NOT implement soft delete: the Vehicle and Driver DELETE endpoints hard-delete the row. Iter 8 introduced the Trip aggregate, which references Vehicle and Driver by id with FK delete policy `Restrict`; from this iter forward, deleting a vehicle or driver that participates in any Trip is blocked at the database (Postgres error 23503, Prisma `P2003`) and surfaces today as a raw 500. The iter-9 Trips write path will add the `P2003 → HTTP 409 "Cannot delete: N trip(s) reference this vehicle"` mapping in the Vehicles and Drivers service layers (this commitment is recorded as the active tech-debt entry "Vehicle/Driver delete must map P2003 to HTTP 409 once Trip write path lands"). The block-when-referenced policy is now the project's choice over true soft delete; a follow-up that adds `deletedAt` to either aggregate would be a separate ADR.
